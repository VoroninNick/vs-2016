class EditedFileEditorController < ApplicationController
  before_filter do
    if !current_user
      render_not_found
    end
  ...
  def index
    ...
    @relative_path = @normalized_path.gsub(/\A#{@root_path}/, "")
    @relative_path_array = @relative_path.split('/').select(&:present?)
    ...
    if @exist
      ...
      @relative_path_array.each_with_index do |entry_name, index|
        entry = {}
        entry[:name] = entry_name
        entry[:full_path] = "/" + @relative_path_array[0, index + 1].join('/')
        entry[:is_directory] = (index == @relative_path_array.count - 1)? @is_directory : true
        entry[:is_file] = (index == @relative_path_array.count - 1)? @is_file : false
        entry[:is_application_root] = index == 0
      ...
      if @is_directory
        ...
        entries.each do |e|
          ...
          entry_url.gsub!(/\A#{@root_path}/, "")



          entry_info = {name: e.split('/').last, full_path: entry_url, is_directory: File.directory?(e), is_file: !File.directory?(e) }
